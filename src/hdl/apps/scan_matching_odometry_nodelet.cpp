...
public:
  ...
  virtual void onInit() {
    ...
    first_lidar_pose_sub = nh.subscribe<geometry_msgs::Pose>("/lidar/first_pose", 1, &ScanMatchingOdometryNodelet::GPSfirst, this);
    subGps = nh.subscribe<geometry_msgs::Point>("/use_lidar/may_or_not", 1, &ScanMatchingOdometryNodelet::GpsHandler, this);
  }
...
private:
  ...
  void GPSfirst(const geometry_msgs::PoseConstPtr& msg){

    Eigen::Quaterniond q1(msg->orientation.w,msg->orientation.x,msg->orientation.y,msg->orientation.z);
    
    //T_init(q1);
    Eigen::Isometry3d T(q1);
    //T.pretranslate(Eigen::Vector3d(msg->position.x,msg->position.y,msg->position.z));
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    if(use_lidar == true)
    {
      ...
      Eigen::Matrix4f pose = matching(cloud_msg->header.stamp, cloud);    //得出的pose已经是相对于起始位置的了!!!
      ...
      //std::cout<<"=========>prev_trans,keyframe_pose has reset to 0!!"<<std::endl;
     
       //std::cout<<"=========>雷达!!"<<std::endl;
    

    }else{
      //std::cout<<"=========>不用雷达!!"<<std::endl;
      return;
    }
...
//if(use_lidar == true)满足条件才会进行ndt匹配;

void GpsHandler(const geometry_msgs::Point msg){
    //std::cout<<"hdl_GpsHandler!!"<<std::endl;
    //int ndt_count;
...
//**************************************************************************************
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(!keyframe && flag_ == false) {            //当进入下一站之前flag_要再次设为false
      Eigen::Matrix<double, 4, 4> temp_trans; 
      temp_trans = T_init.matrix();
      ...
      //prev_trans = prev.cast<float>();    //doble不能转float,需要强制类型转换;
      prev_trans.setIdentity();            
      keyframe_pose = prev.cast<float>();
      //keyframe_pose.setIdentity(); 
      keyframe_stamp = stamp;
      keyframe = downsample(cloud);
      registration->setInputTarget(keyframe);
      flag_ = true;
      //return prev.cast<float>();
      return Eigen::Matrix4f::Identity();
    }

    //std::cout << "匹配初始值" << T_init.matrix() << std::endl;
    ...
    //std::cout << "ndt匹配帧间位姿" << odom << std::endl;与初始位置的
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose) {
    ...
    std::cout<<"odom X= "<<odom.pose.pose.position.x<<" Y= "<<odom.pose.pose.position.y<<" Z= "<<odom.pose.pose.position.z<<std::endl;
  }
...
private:
  ...
  ros::Subscriber first_lidar_pose_sub;
