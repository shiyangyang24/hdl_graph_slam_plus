...
public:
  ...
  virtual void onInit() {
    ...
    gps_edge_stddev_xy = private_nh.param<double>("gps_edge_stddev_xy", 0.000001);
    gps_edge_stddev_z = private_nh.param<double>("gps_edge_stddev_z", 0.001);
    floor_edge_stddev = private_nh.param<double>("floor_edge_stddev", 10.0);
    sim_odom_edge_stddev_xy = private_nh.param<double>("real_odom_edge_stddev_xy", 0.01);
    sim_odom_edge_stddev_z = private_nh.param<double>("real_odom_edge_stddev_z", 0.0001);
    ...
    //imu_sub = nh.subscribe("/gpsimu_driver/imu_data", 1024, &HdlGraphSlamNodelet::imu_callback, this);
     subGps = nh.subscribe<geometry_msgs::Point>("/use_lidar/may_or_not", 1, &HdlGraphSlamNodelet::GpsHandler, this);//---->>>>>
     first_pose_sub = nh.subscribe<geometry_msgs::Pose>("/lidar/first_pose", 1, &HdlGraphSlamNodelet::firstposecallback, this);
     //sim_odom_sub = nh.subscribe<nav_msgs::Odometry>("/simulation/odom", 1, &HdlGraphSlamNodelet::sim_odom_callback, this);



    imu_sub = nh.subscribe("/mavros/imu/data", 1024, &HdlGraphSlamNodelet::imu_callback, this);
    ...
    lidar_pose_pub = mt_nh.advertise<geometry_msgs::PoseStamped>("/lidar_XYZ", 32);
...
//************************************************************************************
  /**
   * @brief GpsHandler
   * @param watrix_vp::GPS_IMU gps
   * @param none
   */


void firstposecallback(const geometry_msgs::PoseConstPtr& msg){

    Eigen::Quaterniond q1(msg->orientation.w,msg->orientation.x,msg->orientation.y,msg->orientation.z);
    
    //T_init(q1);
    Eigen::Isometry3d T(q1);
...
void GpsHandler(const geometry_msgs::Point msg){
    ...
    if(msg.x == 1)
       ...
       if(false == use_lidar)
       {
          ...
          use_lidar = true;
          //std::cout<<"graph_slam=========>use_lidar!!"<<std::endl;
       }
      ...
      return;
          ...
          //std::cout<<"graph_slam=========>sys_reset();!!"<<std::endl;
          //count_gps=0;
          ...
          if(use_lidar == true){
            ...
            boost::optional<Eigen::Vector3d> zero1;
...
//**************************************************************************************
   ...
   */
    void sys_reset()
    {
            keyframes.clear();              //std::vector<KeyFrame::Ptr> keyframes;  也是关键帧的集合不过是向量
            keyframe_queue.clear();         //关键帧集合双端队列
            floor_coeffs_queue.clear();
            imu_queue.clear();
            gps_queue.clear();

            graph_slam->graph->vertices().clear();
	          graph_slam->graph->edges().clear();



            new_keyframes.clear();  // 回环检测用的

            keyframes_snapshot.clear();       //   std::vector<KeyFrameSnapshot::Ptr> keyframes_snapshot;  用于产生地图的,会有延迟
            map_cloud_generator.reset();       //std::unique_ptr<MapCloudGenerator> map_cloud_generator;

           
            keyframe_hash.clear();           //  std::unordered_map<ros::Time, KeyFrame::Ptr, RosTimeHash> keyframe_hash;
...
//**************************************************************************************
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    //*/
     ...
     //里程计模拟值回调
  /*
  void sim_odom_callback(const nav_msgs::OdometryConstPtr& sim_msg){
        std::lock_guard<std::mutex> lock1(sim_odom_queue_mutex);
        //sim_msg->header.stamp = real_odom_msg->header.stamp + ros::Duration(real_odom_time_offset);
        sim_odom_queue.push_back(sim_msg);
    }
  ...
  */
  ...
  bool flush_keyframe_queue() {
    ...
    if(keyframe_queue.empty()) {   //判断sim_odom_queue是否为空加在下面sim_odom作为边的图优化中
      return false;
    }
    ...
    for(int i=0; i<std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      // add pose node
      Eigen::Isometry3d odom = odom2map * keyframe->odom;    //初始化odom2map是单位阵
      keyframe->node = graph_slam->add_se3_node(odom);      //将odom设为节点并且赋值给keyframe->node,当做节点
      keyframe_hash[keyframe->stamp] = keyframe;            //设置了一个哈希表,是一个map容器
      ...
      if(keyframes.empty() && new_keyframes.size() == 1) {
        if(private_nh.param<bool>("fix_first_node", false)) {
          ...
          Eigen::Isometry3d relative_pose = keyframe->odom.inverse();
          //首次位置的优化
          anchor_edge = graph_slam->add_se3_edge(anchor_node, keyframe->node, relative_pose, Eigen::MatrixXd::Identity(6, 6));
          
        }
      }
      //std::cout << "第一个点" << anchor_edge->vertices()[0] << std::endl;
      //std::cout << graph_slam->vertices()[0];

      if(i==0 && keyframes.empty()) {
        continue;
      }

      // add edge between consecutive keyframes  :: ndt匹配的结果
  ...
  bool flush_gps_queue() {
    ...
    for(auto& keyframe : keyframes) {
      ...
      // the first gps data position will be the origin of the map    也是把相对位置偏差作为边来优化,只是没有加入角度
      if(!zero_utm) {
        zero_utm = xyz;
      }
      xyz -= (*zero_utm);       //x,y,z是与原点的差值

      std::cout << "纯惯导推算值" << xyz << std::endl;

      keyframe->utm_coord = xyz + Eigen::Vector3d(50,0,0);
...
//我们根据机器人位姿来作为图的顶点，这个位姿可以来自机器人的编码器，也可以是ICP匹配得到的，图的边就是位姿之间的关系。
//现在里程计当边还是当顶点.
/***************************************************************
  bool flush_odom_queue() {
    std::lock_guard<std::mutex> lock(sim_odom_queue_mutex);

    if(keyframes.empty() || sim_odom_queue.empty()) {
      return false;
    }

    bool updated = false;
    auto sim_odom_cursor = sim_odom_queue.begin();

    for(auto& keyframe : keyframes) {
      if(keyframe->stamp > sim_odom_queue.back()->header.stamp) {
        break;
      }

      if(keyframe->stamp < (*sim_odom_cursor)->header.stamp) {
        continue;
      }

      // find odom data which is closest to the keyframe
      auto closest_sim_odom = sim_odom_cursor;
      for(auto sim_odom = sim_odom_cursor; sim_odom != sim_odom_queue.end(); sim_odom++) {

        auto dt = ((*closest_sim_odom)->header.stamp - keyframe->stamp).toSec();
        auto dt2 = ((*sim_odom)->header.stamp - keyframe->stamp).toSec();

        if(std::abs(dt) < std::abs(dt2)) {
          break;
        }

        closest_sim_odom = sim_odom;
      }

      sim_odom_cursor = closest_sim_odom;
      if(0.2 < std::abs(((*closest_sim_odom)->header.stamp - keyframe->stamp).toSec())) {
        continue;
      }

      //里程计的速度是相对于baselink的,位置是相对于轮子中心的;
      //?转换到base坐标系下?   只需要一个平移   不需要

      Eigen::Vector3d odom_xyz((*closest_sim_odom)->pose.pose.position.x, (*closest_sim_odom)->pose.pose.position.y, (*closest_sim_odom)->pose.pose.position.z);
      //将里程计作为一个边来约束
      keyframe->odom_coord = odom_xyz;


      g2o::OptimizableGraph::Edge* edge;
      
      //信息矩阵;权重矩阵
      Eigen::Matrix3d odom_information_matrix = Eigen::Matrix3d::Identity();
      odom_information_matrix.block<2, 2>(0, 0) /= sim_odom_edge_stddev_xy;
      odom_information_matrix(2, 2) /= sim_odom_edge_stddev_z;
      ...
      //
      //添加边
      edge = graph_slam->add_se3_point_xyz_edge(keyframe->node,keyframes.back()->node, odom_xyz, odom_information_matrix);
      
      graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("sim_odom_edge_robust_kernel", "NONE"), private_nh.param<double>("sim_odom_edge_robust_kernel_size", 1.0));
    ...
    //将已经优化的odom从队列中移除;
    auto remove_loc = std::upper_bound(sim_odom_queue.begin(), sim_odom_queue.end(), keyframes.back()->stamp,
      [=](const ros::Time& stamp, const nav_msgs::OdometryConstPtr& sim_odom) {
        return stamp < sim_odom->header.stamp;
      }
    );
    sim_odom_queue.erase(sim_odom_queue.begin(), remove_loc);    
    return updated;
...
//// add edge between consecutive keyframes    ::   里程计计算的结果
    //时间戳找的不对吗?
  ...
  bool flush_odom_queue() {
      ...
      if(!sim_odom_queue.empty()){
          ...
          for(auto& keyframe__ : keyframes) {
            ...
            if(keyframe__->stamp > sim_odom_queue.back()->header.stamp) {
              break;
            }

            if(keyframe__->stamp < (*sim_odom_cursor)->header.stamp) {
              i++;
              continue;
            }
            ...
            for(auto sim_odom = sim_odom_cursor; sim_odom != sim_odom_queue.end(); sim_odom++) {
              ...
              auto dt = ((*closest_sim_odom)->header.stamp - keyframe__->stamp).toSec();
              auto dt2 = ((*sim_odom)->header.stamp - keyframe__->stamp).toSec();
            ...
            sim_odom_cursor = closest_sim_odom;
            if(0.2 < std::abs(((*closest_sim_odom)->header.stamp - keyframe__->stamp).toSec())) {
              continue;
            }
            ...
            Eigen::Vector3d odom_xyz_((*closest_sim_odom)->pose.pose.position.x, (*closest_sim_odom)->pose.pose.position.y, (*closest_sim_odom)->pose.pose.position.z);
            odom_xyz = odom_xyz_;
            //将里程计作为一个边来约束
            keyframe__->odom_coord = odom_xyz;

            keyframe_cur = keyframe__;
            keyframe_pre = keyframes[i-1];

            Eigen::Isometry3d T_sim_odom = keyframe_cur->odom;
            T_sim_odom.pretranslate(odom_xyz);

            //Eigen::Isometry3d relative_pose1 = T_sim_odom.inverse() * keyframe_pre->odom;
            //Eigen::MatrixXd information1 = 5* inf_calclator->calc_information_matrix(keyframe_pre->cloud, keyframe_cur->cloud, relative_pose1);
            //auto edge1 = graph_slam->add_se3_edge(keyframe_cur->node, keyframe_pre->node, relative_pose1, information1);
            //graph_slam->add_robust_kernel(edge1, private_nh.param<std::string>("sim_odom_edge_robust_kernel", "NONE"), private_nh.param<double>("sim_odom_edge_robust_kernel_size", 1.0));
            
            updated = true;


          }//找最近的odom与keyframe循环结束;
      }
          ...
          /*找到离着keyframe_cur最近的上一个关键帧;
          for(auto& keyframe1 : keyframes) {
            if(keyframe1->stamp > keyframe_cur->stamp) {
              break;
            }
            auto closest_keyframe_cur = keyframe1;
            auto dt3 = (keyframe_cur->stamp - closest_keyframe_cur->stamp).toSec();
            ...
            //double dt4;
            //auto dt4 = dt3;    //初始化
            if(std::abs(dt4) >= std::abs(dt3)){
              keyframe_pre = closest_keyframe_cur;
              dt4 = dt3;
            ...
            if(0.2 < std::abs((keyframe_cur->stamp - keyframe_pre->stamp).toSec())) {
              keyframe_pre = NULL;
              continue;
            }
          }
            ...
            //计算信息矩阵
            //Eigen::Isometry3d relative_pose1(loop->relative_pose.cast<double>());
            //Eigen::MatrixXd information_matrix = inf_calclator->calc_information_matrix(loop->key1->cloud, loop->key2->cloud, relpose);

            //if(keyframe_pre != NULL && keyframe_cur != NULL){
  ...
  */
  ...
  void imu_callback(const sensor_msgs::ImuPtr& imu_msg) {
   ...
   {
      ROS_INFO("imu get!! %f,%f,%f" ,(*imu_msg).linear_acceleration.x ,(*imu_msg).linear_acceleration.y,(*imu_msg).linear_acceleration.z);

    //  float temp = (*imu_msg).linear_acceleration.x;
    //  float temp2 = (*imu_msg).angular_velocity.x;
    //  (*imu_msg).linear_acceleration.x = (*imu_msg).linear_acceleration.y;      (*imu_msg).angular_velocity.x = (*imu_msg).angular_velocity.y;
    //  (*imu_msg).linear_acceleration.y = temp;                                  (*imu_msg).angular_velocity.y = temp2;
    //  (*imu_msg).linear_acceleration.z = -1 * (*imu_msg).linear_acceleration.z; (*imu_msg).angular_velocity.z = -1 * (*imu_msg).angular_velocity.z;




    
    //  (*imu_msg).linear_acceleration.y = -1 * (*imu_msg).linear_acceleration.y;                        
    //  (*imu_msg).linear_acceleration.z = -1 * (*imu_msg).linear_acceleration.z; 

    //  (*imu_msg).angular_velocity.y = -1 * (*imu_msg).angular_velocity.y;
    //  (*imu_msg).angular_velocity.z = -1 * (*imu_msg).angular_velocity.z;
...
*/
    ...
    //publish 优化后的位姿;
    geometry_msgs::PoseStamped increment_lidar_p;

    increment_lidar_p.header.stamp = ros::Time::now();
    increment_lidar_p.pose.position.x = keyframe->node->estimate().translation().x();
    increment_lidar_p.pose.position.y = keyframe->node->estimate().translation().y();
    increment_lidar_p.pose.position.z = keyframe->node->estimate().translation().z();
    //increment_lidar_p.orientation.x = keyframe->node->estimate().rotation().x();
    Eigen::AngleAxisd rotation_vector(keyframe->node->estimate().rotation());
    Eigen::Quaterniond quaternion(keyframe->node->estimate().rotation());
    ::Eigen::Quaterniond q;   
    q = keyframe->node->estimate().rotation();      
    //std::cout << "四元数" << q.x() << " he " << quaternion.x() << std::endl;
    increment_lidar_p.pose.orientation.x = q.x();
    increment_lidar_p.pose.orientation.y = q.y();
    increment_lidar_p.pose.orientation.z = q.z();
    increment_lidar_p.pose.orientation.w = q.w();
    ...
    //std::cout<<"keyframes.size()="<<keyframes.size()<<std::endl;   keyframes[i]->node->estimate().translation()
   //std::cout<<"keyframes->node->estimate()= "<<keyframes[keyframes.size()-1]->node->estimate().translation().x()<<" Y= "<<keyframes[keyframes.size()-1]->node->estimate().translation().y()<<" Z= "<<keyframes[keyframes.size()-1]->node->estimate().translation().z()<<std::endl;
   ROS_INFO("keyframe!! %f,%f,%f" ,keyframe->node->estimate().translation().x(),keyframe->node->estimate().translation().y(),keyframe->node->estimate().translation().z());
//std::cout<<"keyframes->node->estimate()= "<<keyframes[0]->node->estimate().translation().x()<<" Y= "<<keyframes[0]->node->estimate().translation().y()<<" Z= "<<keyframes[0]->node->estimate().translation().z()<<std::endl;
    
    
    
    if(lidar_pose_pub.getNumSubscribers()) {
      lidar_pose_pub.publish(increment_lidar_p);
    }
    ...
    if(odom2map_pub.getNumSubscribers()) {
      ...
      odom2map_pub.publish(ts);
       ...
       //std::cout<<"translation X= "<<ts.transform.translation.x<<" Y= "<<ts.transform.translation.y<<" Z= "<<ts.transform.translation.z<<std::endl;
       //std::cout<<"translation X= "<<ts.transform.translation.x<<" Y= "<<ts.transform.translation.y<<" Z= "<<ts.transform.translation.z<<std::endl;
    }
...
private:
  ...
  //zhen
  KeyFrame::Ptr keyframe_cur;
  KeyFrame::Ptr keyframe_pre;
  double dt4;
  // ROS
  ros::NodeHandle nh;
  ...
  ros::Publisher lidar_pose_pub;   //图优化之后的pose发布
  ...
  //sim odom queue
  std::mutex sim_odom_queue_mutex;
  std::deque<nav_msgs::OdometryConstPtr> sim_odom_queue;
  double sim_odom_edge_stddev_xy;
  double sim_odom_edge_stddev_z;  
